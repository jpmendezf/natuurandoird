...
let ProfilePage = class ProfilePage {
    constructor(ntrl, api, actionSheetCtrl, camera, util, route, translate) {
        this.ntrl = ntrl;
        this.api = api;
        ...
        this.util.startLoad();
        this.api.getDataWithToken("viewReview").subscribe((res) => {
            if (res.success) {
                this.data = res.data;
    ...
    editProfile() {
        if (this.segment == 3) {
            this.util.startLoad();
            this.api.postDataWithToken("editProfile", this.userDetail).subscribe((res) => {
                if (res.success) {
                    this.err = {};
                    this.util.dismissLoader();
            ...
            this.util.startLoad();
            this.api.postDataWithToken("changePassword", this.passwordData).subscribe((res) => {
                if (res.success) {
                    this.util.dismissLoader();
