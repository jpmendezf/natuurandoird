...
let CategoryPage = class CategoryPage {
    constructor(api, util, geolocation, ntrl, route) {
        this.api = api;
        this.util = util;
        ...
        this.route.params.subscribe(params => {
            ...
            if (this.isfrom) {
                ...
                this.util.startLoad();
                this.api.getDataWithToken("shops").subscribe((res) => {
                    if (res.success) {
                        this.data = res.data;
                        if (this.api.filterType == "popular") {
                            ...
                            if (localStorage.getItem("isaddress") != "false") {
                                this.api
                                    ...
                                    if (res.success) {
                                        this.data.forEach(element => {
                                            element.distance = this.distance(res.data.lat, res.data.lang, element.latitude, element.longitude, "K");
                                        });
                                        this.data.sort((a, b) => {
                                ...
                                this.geolocation
                                    ...
                                    this.data.forEach(element => {
                                        element.distance = this.distance(resp.coords.latitude, resp.coords.longitude, element.latitude, element.longitude, "K");
                                    });
                                    this.data.sort((a, b) => {
                                        if (a.distance < b.distance) {
                                            return -1;
