...
var PaymentMethodPage = /** @class */ (function () {
    function PaymentMethodPage(ntrl, modalController, api, util, payPal, translate) {
        var _this = this;
        this.ntrl = ntrl;
        ...
        this.util.startLoad();
        this.api.getDataWithToken("keySetting").subscribe(function (res) {
            if (res.success) {
                _this.razor_key = res.data.razorPublishKey;
                _this.paypalProductionkey = res.data.paypalProduction;
                _this.paypalSanboxkey = res.data.paypalSendbox;
                _this.util.dismissLoader();
            }
        });
    }
    PaymentMethodPage.prototype.ngOnInit = function () { };
    PaymentMethodPage.prototype.paymentMethod = function () {
        var _this = this;
        this.data.items = [];
        ...
        this.api.cartData.cartData.forEach(function (element) {
            if (element.type == "combo") {
                _this.data.package_id.push(element.id);
                var pusher = {
                    item: "",
                    price: element.total,
                ...
                _this.data.itemData.push(pusher);
            }
            else {
                _this.data.items.push(element.id);
                var pusher = {
                    item: element.id,
                    price: element.total,
                    quantity: element.qty,
        ...
        if (this.online) {
            ...
            this.util.startLoad();
            this.api.postDataWithToken("createOrder", this.data).subscribe(function (res) {
                if (res.success) {
                    _this.api.promocode = {};
                    _this.util.dismissLoader();
                    _this.api.checkOrderStatus = res.data.id;
                    _this.presentModal();
                }
            }, function (err) {
    ...
    PaymentMethodPage.prototype.back = function () {
        this.ntrl.back();
    };
    PaymentMethodPage.prototype.presentModal = function () {
    ...
    PaymentMethodPage.prototype.payWithRazor = function () {
        var _this = this;
        var options = {
        ...
        var successCallback = function (payment_id) {
            _this.data.payment_token = payment_id;
            _this.data.payment_status = 1;
            _this.data.payment_type = "RAZOR";
            _this.util.startLoad();
            _this.api.postDataWithToken("createOrder", _this.data).subscribe(function (res) {
                if (res.success) {
                    _this.api.promocode = {};
                    _this.util.dismissLoader();
                    _this.api.checkOrderStatus = res.data.id;
    ...
    PaymentMethodPage.prototype.paypalPay = function () {
        var _this = this;
        this.payPal
            .init({
            ...
            _this.payPal
                ...
                .then(function () {
                var payment = new _ionic_native_paypal_ngx__WEBPACK_IMPORTED_MODULE_6__["PayPalPayment"](_this.data.payment, _this.currencyType, "Description", "sale");
                _this.payPal.renderSinglePaymentUI(payment).then(function (result) {
                    _this.data.payment_token = result.response.id;
                    _this.data.payment_status = 1;
                    _this.data.payment_type = "PAYPAL";
                    _this.util.startLoad();
                    _this.api
                        .postDataWithToken("createOrder", _this.data)
    ...
    PaymentMethodPage.ctorParameters = function () { return [
