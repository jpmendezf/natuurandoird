...
var TimelinePage = /** @class */ (function () {
    function TimelinePage(translate, ntrl, api, util, alertController) {
        var _this = this;
        ...
        this.api
            ...
            if (res.success) {
                ...
                _this.getShopAddress();
                _this.util.dismissLoader();
                if (_this.data.deliveryBoy_id != null) {
                    _this.isvisible = true;
                    _this.driver = {
                        lat: parseFloat(res.data.driver.lat),
                        lng: parseFloat(res.data.driver.lang),
                    };
                    _this.driver.name = res.data.driver.name;
                    _this.driver.imagePath = res.data.driver.imagePath;
                    _this.driver.image = res.data.driver.image;
                    _this.driver.phone = res.data.driver.phone;
                    _this.isvisible = true;
                    _this.Centerlat = parseFloat(_this.driver.lat);
                    _this.Centerlng = parseFloat(_this.driver.lang);
                    if (_this.data.order_status == "PickUpFood" ||
                        _this.data.order_status == "OnTheWay" ||
                        _this.data.order_status == "DriverReach" ||
                        _this.data.order_status == "Delivered") {
                        _this.markerOptions.origin.icon = "../../../assets/image/bike.png";
                        _this.isvisible = false;
                    }
                }
                else {
                    _this.Centerlat = parseFloat(_this.origin.lat);
                    _this.Centerlng = parseFloat(_this.origin.lng);
                }
                if (_this.data.order_status == "Pending" ||
                    _this.data.order_status == "Approved" ||
                    _this.data.order_status == "DriverApproved") {
                    _this.status = "1";
                }
                else if (_this.data.order_status == "Prepare") {
                    _this.status = "2";
                }
                else if (_this.data.order_status == "PickUpFood" ||
                    _this.data.order_status == "DriverAtShop") {
                    _this.status = "3";
                }
                else if (_this.data.order_status == "OnTheWay") {
                    _this.isvisible = false;
                    _this.markerOptions.origin.icon = "../../../assets/image/bike.png";
                    _this.status = "4";
                }
                else if (_this.data.order_status == "DriverReach") {
                    _this.status = "5";
                    _this.markerOptions.origin.icon = "../../../assets/image/bike.png";
                    _this.isvisible = false;
                }
        ...
        this.get_duration_interval = setInterval(function (interval) {
            if (_this.data.order_status == "Delivered") {
                clearInterval(_this.get_duration_interval);
            }
            else {
                _this.DriverLocation();
            }
        }, this.api.request_duration);
    }
    TimelinePage.prototype.cancleOrder = function () {
                                        ...
                                        buttons: [
                                            {
                                                ...
                                                handler: function () {
                                                    ...
                                                    _this.api
                                                        .getDataWithToken("cancelOrder/" + _this.data.id)
                                                        .subscribe(function (res) {
                                                        if (res.success) {
                                                            _this.util.dismissLoader();
                                                            _this.util.presentToast(res.msg);
                                                            _this.ntrl.navigateRoot("/order-history");
                                                        }
                                                        else {
                                                            _this.util.dismissLoader();
                                                            _this.util.presentToast(res.msg);
    ...
    TimelinePage.prototype.orderHistory = function () {
        this.ntrl.navigateForward(["order-history"]);
    };
    TimelinePage.prototype.DriverLocation = function () {
        var _this = this;
        this.api.getDataWithToken("driverLocation/" + this.data.id).subscribe(function (res) {
            if (res.success) {
                _this.driver = {
                ...
                _this.Centerlat = parseFloat(res.data.lat);
                _this.Centerlng = parseFloat(res.data.lang);
                if (_this.data.deliveryBoy_id == null) {
                    _this.isvisible = false;
                }
                if (res.data.order_status == "DriverApproved") {
                    _this.status = "1";
                    _this.data.order_status = res.data.order_status;
                    _this.isvisible = true;
                }
    ...
    TimelinePage.prototype.getUserAddress = function () {
        var _this = this;
                ...
                if (res.success) {
                    _this.userAddress =
                        res.data.soc_name +
                            ...
                            " " +
                            res.data.zipcode;
                    _this.geocoder.geocode({ address: _this.userAddress }, function (results, status) {
    ...
    TimelinePage.prototype.getShopAddress = function () {
        this.origin = {
            lat: parseFloat(this.data.shop.latitude),
            lng: parseFloat(this.data.shop.longitude),
        };
    };
    TimelinePage.prototype.ionViewWillLeave = function () {
...
//# sourceMappingURL=pages-timeline-timeline-module-es5.js.map